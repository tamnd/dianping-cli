...
describes what users get. Pick a format with `-o`, or let dp choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
dp <command> -o table   # aligned columns for reading
dp <command> -o jsonl   # one JSON object per line, for piping
dp <command> -o json    # a single JSON array
dp <command> -o csv     # spreadsheet friendly
dp <command> -o tsv     # tab-separated
dp <command> -o url     # just the URL column
dp <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
dp <command> --fields id,title,url
```
...
```bash
dp <command>            # a table, because this is a terminal
dp <command> | wc -l    # JSONL, because this is a pipe
```
